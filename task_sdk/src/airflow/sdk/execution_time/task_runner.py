...
def run(
    ...
    try:
        ...
        msg = DeferTask(
            ...
            method_name=defer.method_name,
            kwargs=defer.kwargs or {},
        )
...
def _execute_task(context: Context, ti: RuntimeTaskInstance):
    ...
    if ti._ti_context_from_server and ti._ti_context_from_server.next_method:
        ...
        kwargs = BaseSerialization.deserialize(ti._ti_context_from_server.next_kwargs)
        execute = functools.partial(getattr(task, ti._ti_context_from_server.next_method), **kwargs)
